...
def expand_prompts_for_mesh(prompts, global_device_count):
    if len(prompts) == 0:
        raise ValueError("Need at least one prompt for inference.")
    if global_device_count <= 0:
        raise ValueError("Global device count must be positive.")

    original_prompt_count = len(prompts)
    padded_prompt_count = (
        math.ceil(original_prompt_count / global_device_count) * global_device_count
    )
    expanded_prompts = list(prompts)

    while len(expanded_prompts) < padded_prompt_count:
        prompt_index = len(expanded_prompts) % original_prompt_count
        expanded_prompts.append(prompts[prompt_index])

    return expanded_prompts, original_prompt_count
...
if __name__ == "__main__":
    ...
    warmup_prompts = ["<|endoftext|>Did you hear the noise coming "] * global_device_count
    encoded = tokenizer.encode_batch(warmup_prompts, allowed_special="all")
    input_ids, segment_ids = pad_tokens(encoded, pad_id=PAD_ID, pad_to_power_of_two=True)
    ...
    warmup_cache = KVCache.init(cache_key, cfg.mesh, cfg.rules, batch_size, cfg)
    ...
    with set_mesh(cfg.mesh):
        ...
        _, next_tokens, warmup_cache = prefill(
            model, input_ids, segment_ids, warmup_cache, head_dim, pad_id=PAD_ID
        )
        ...
        warmup_last_token = next_tokens[:, None]
        generate(
            model,
            warmup_cache,
            warmup_last_token,
    ...
    expanded_prompts, original_prompt_count = expand_prompts_for_mesh(
        prompts, global_device_count
    )
    encoded = tokenizer.encode_batch(expanded_prompts, allowed_special="all")
    input_ids, segment_ids = pad_tokens(encoded, pad_id=PAD_ID, pad_to_power_of_two=True)
    batch_size = input_ids.shape[0]
    cache_key = jax.random.PRNGKey(2)
    ...
    gathered_generated = np.asarray(process_allgather(generated, tiled=True))
    prompts = prompts[:original_prompt_count]
    if process_index == 0:
        decoded = tokenizer.decode_batch(gathered_generated.tolist())[:original_prompt_count]
        print(
